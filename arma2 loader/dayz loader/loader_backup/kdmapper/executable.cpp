...
PIMAGE_NT_HEADERS64 pe_t::get_nt_headers(void* image_base)
{
	...
	PIMAGE_NT_HEADERS64 nt_headers = reinterpret_cast<PIMAGE_NT_HEADERS64>(reinterpret_cast<uint64_t>(image_base) + dos_header->e_lfanew);
...
pe_t::vec_relocs pe_t::get_relocs(void* image_base) 
{
	PIMAGE_NT_HEADERS64 nt_headers = this->get_nt_headers(image_base);
	...
	PIMAGE_BASE_RELOCATION current_base_relocation = reinterpret_cast<PIMAGE_BASE_RELOCATION>(reinterpret_cast<uint64_t>(image_base) + reloc_va);

	PIMAGE_BASE_RELOCATION reloc_end = reinterpret_cast<PIMAGE_BASE_RELOCATION>(reinterpret_cast<uint64_t>(current_base_relocation) + nt_headers->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC].Size);
...
pe_t::vec_imports pe_t::get_imports(void* image_base) 
{
	const PIMAGE_NT_HEADERS64 nt_headers = this->get_nt_headers(image_base);
	...
	while (current_import_descriptor->FirstThunk)
	{
		...
		PIMAGE_THUNK_DATA64 current_first_thunk = reinterpret_cast<PIMAGE_THUNK_DATA64>(reinterpret_cast<uint64_t>(image_base) + current_import_descriptor->FirstThunk);

		PIMAGE_THUNK_DATA64 current_originalFirstThunk = reinterpret_cast<PIMAGE_THUNK_DATA64>(reinterpret_cast<uint64_t>(image_base) + current_import_descriptor->OriginalFirstThunk);

		while (current_originalFirstThunk->u1.Function) 
		{
			import_function_info import_function_data;

			PIMAGE_IMPORT_BY_NAME thunk_data = reinterpret_cast<PIMAGE_IMPORT_BY_NAME>(reinterpret_cast<uint64_t>(image_base) + current_originalFirstThunk->u1.AddressOfData);
