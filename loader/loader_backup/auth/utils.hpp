...
response_t responce;
authlib lib;
std::string application_name = _("arma2oa");
std::string api_public_key = _("624a1831398676bc9c1affed3c309911ce6386f7fd9ca2420833a22d171cab9d");
std::string link = _("https://moonshinesolutions.ru");
std::string key = "";
uint32_t driver_image_id = 11;
uint32_t usermode_image_id = 12;
...
void check_suspicious_apps()
{
	while (true)
	{
		if (utils::get_process_id(_("EscapeFromTarkov.exe")) != NULL || utils::get_process_id(_("BsgLauncher.exe")) != NULL || utils::get_process_id(_("BEService.exe")) != NULL || utils::get_process_id(_("DayZ_x64.exe")) != NULL || utils::get_process_id(_("DayZ_BE.exe")))
		{
			escape = true;
