...
{
    uint32_t offset1 = this->pdb.get_rva(xor (L"MmAllocateIndependentPagesEx"));

    uint32_t offset2 = this->pdb.get_rva(xor (L"MmSetPageProtection"));
    
    uint64_t offset3 = this->pdb.get_rva(xor (L"ExAllocatePoolWithTag"));
...
uint64_t mapper_t::allocate_big_pool(uint32_t size)
{
    ...
    if (!this->call_kernel_function(&allocated_pool, kernel_ExAllocatePool, 0, size, 'cimf')) //Changed pool tag since an extremely meme checking diff between allocation size and average for detection....
        return 0;
    if (!this->call_kernel_function(&psize_t, kernel_ExAllocatePool, 0, align(0x1000), 'SIze'))
        return 0;
    int status = this->call_kernel_function(PVOID(NULL), RtlZeroMemory, psize_t, align(0x1000));
...
//    if (!WdFilter)
//    {
//        output->success(xor ("[!] wdfilter.sys not found. \n"));
//
//        output->success(xor ("[!] successfull setup! \n"));
//
//        return true;
//    }
//    else
//    {
//        output->error(_("[-] wdfilter.sys found on your system, use defender control to cut defender off\n"));
//        Sleep(5000);
//    }
...
bool mapper_t::clear_wdfilter_driver_list() {
    ...
    auto WdFilter = this->utils.get_kernel_module_base(_("WdFilter.sys"));
    if (!WdFilter) {
        //log(_("[+] wdfilter not found!\n"));
        return true;
    }

    auto RuntimeDriversList = this->find_pattern_in_section_in_kernel(_("PAGE"), WdFilter, (PUCHAR)"\x48\x8B\x0D\x00\x00\x00\x00\xFF\x05", _("xxx????xx"));
    if (!RuntimeDriversList) {
        log(_("[!] failed to find wdfilter runtimedriverslist\n"));
        return false;
    }

    auto RuntimeDriversCountRef = this->find_pattern_in_section_in_kernel( _("PAGE"), WdFilter, (PUCHAR)"\xFF\x05\x00\x00\x00\x00\x48\x39\x11", _("xx????xxx"));
    if (!RuntimeDriversCountRef) {
        log(_("[!] failed to find wdfilter RuntimeDriversCount\n"));
        return false;
    }
    ...
    */
    auto MpFreeDriverInfoExRef = this->find_pattern_in_section_in_kernel( _("PAGE"), WdFilter, (PUCHAR)"\x49\x8B\xC9\x00\x89\x00\x08\xE8\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xE9", _("xxx?x?xx???????????x"));
    if (!MpFreeDriverInfoExRef) {
        ...
        */
        MpFreeDriverInfoExRef = this->find_pattern_in_section_in_kernel(_("PAGE"), WdFilter, (PUCHAR)"\x48\x89\x4A\x00\x49\x8b\x00\xE8\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xE9", _("xxx?xx?x???????????x"));
        if (!MpFreeDriverInfoExRef) {
            log(_("[!] failed to find wdfilter MpFreeDriverInfoEx\n"));
            return false;
        }
        else {
            log(_("[+] found MpFreeDriverInfoEx on second pattern\n"));
        }
    ...
    RuntimeDriversList = (uintptr_t)this->resolve_relative_address((PVOID)RuntimeDriversList, 3, 7);
    uintptr_t RuntimeDriversList_Head = RuntimeDriversList - 0x8;
    uintptr_t RuntimeDriversCount = (uintptr_t)this->resolve_relative_address((PVOID)RuntimeDriversCountRef, 2, 6);
    uintptr_t RuntimeDriversArray = RuntimeDriversCount + 0x8;
    this->core.read_virtual(RuntimeDriversArray, &RuntimeDriversArray, sizeof(uintptr_t));
    uintptr_t MpFreeDriverInfoEx = (uintptr_t)this->resolve_relative_address((PVOID)MpFreeDriverInfoExRef, 1, 5);

    auto ReadListEntry = [&](uintptr_t Address) -> LIST_ENTRY* { // Usefull lambda to read LIST_ENTRY
        LIST_ENTRY* Entry;
        if (!this->core.read_virtual(Address, &Entry, sizeof(LIST_ENTRY*))) return 0;
        return Entry;
        };
        ...
        Entry = ReadListEntry((uintptr_t)Entry + (offsetof(struct _LIST_ENTRY, Flink))))
    {
        ...
        if (this->core.read_virtual((uintptr_t)Entry + 0x10, &Unicode_String, sizeof(UNICODE_STRING))) {
            auto ImageName = std::make_unique<wchar_t[]>((ULONG64)Unicode_String.Length / 2ULL + 1ULL);
            if (this->core.read_virtual((uintptr_t)Unicode_String.Buffer, ImageName.get(), Unicode_String.Length)) {
                std::wstring driver_name = std::wstring(this->name.begin(), this->name.end());
                if (wcsstr(ImageName.get(), driver_name.c_str())) {
                    ...
                    for (int k = 0; k < 256; k++) { // max RuntimeDriversArray elements
                        PVOID value = 0;
                        this->core.read_virtual(RuntimeDriversArray + (k * 8), &value, sizeof(PVOID));
                        if (value == SameIndexList) {
                            PVOID emptyval = (PVOID)(RuntimeDriversCount + 1); // this is not count+1 is position of cout addr+1
                            this->core.write_virtual(RuntimeDriversArray + (k * 8), &emptyval, sizeof(PVOID));
                    ...
                    if (!removedRuntimeDriversArray) {
                        log(_("[!] failed to remove from RuntimeDriversArray\n"));
                        return false;
                    }
                    ...
                    this->core.write_virtual(uintptr_t(NextEntry) + (offsetof(struct _LIST_ENTRY, Blink)), &PrevEntry, sizeof(LIST_ENTRY::Blink));
                    this->core.write_virtual(uintptr_t(PrevEntry) + (offsetof(struct _LIST_ENTRY, Flink)), &NextEntry, sizeof(LIST_ENTRY::Flink));


                    // decrement RuntimeDriversCount
                    ULONG current = 0;
                    this->core.read_virtual( RuntimeDriversCount, &current, sizeof(ULONG));
                    current--;
                    this->core.write_virtual( RuntimeDriversCount, &current, sizeof(ULONG));
                    ...
                    USHORT Magic = 0;
                    this->core.read_virtual( DriverInfo, &Magic, sizeof(USHORT));
                    if (Magic != 0xDA18) {
                        log(_("[!] DriverInfo Magic is invalid, new wdfilter version?, driver info will not be released to prevent bsod\n"));
                    }
                    else {
                        this->call_kernel_function<void>(nullptr, MpFreeDriverInfoEx, DriverInfo);
                    }

                    log(_("[+] WdFilterDriverList Cleaned:\n"));
                    return true;
                }
            }
        }
...
{
    uint32_t offset1 = this->pdb.get_rva(xor (L"MmFreeIndependentPages"));
...
bool mapper_t::map_bytes(uint8_t* bytes, uint64_t arg1, uint64_t arg2, uint64_t arg3, bool free)
{
    ...
    uint64_t kernel_image_base = this->allocate_big_pool(image_size); //this->allocate_independent_pages(image_size);
    ...
    do
    {
        ...
        if (this->call_kernel_function(&status, address_of_entry_point, arg1, arg2, arg3)) //
        {
...
bool mapper_t::map(std::vector<uint8_t>& encrypted_image, uint64_t arg1, uint64_t arg2, uint64_t arg3, bool destroy_header, bool free)
{
    ...
    if (!this->pdb.load(xor(L"C:\\windows\\system32\\ntoskrnl.exe")))
    {
    ...
    {
        output->error(xor ("[-]ERR_4. failed to start service.\n"));
        ...
        {
            output->error(xor ("[WARNING] unsafe exit. REBOOT PC\n"));
            output->error(xor ("[WARNING] unsafe exit. REBOOT PC\n"));
            output->error(xor ("[WARNING] unsafe exit. REBOOT PC\n"));
        }
    ...
    {
        output-> error(xor ("[-] failed to connect to driver.\n"));
    ...
    if (!this->clear_piddb_cache_table() || !this->clear_mm_unloaded_drivers() || !this->clear_kernel_hash_bucket_list() || !this->clear_wdfilter_driver_list())
    {
        ...
        output->error(xor ("[-]ERR_5 failed to clear traces.\n"));
    ...
    //log(xor ("[+] traces cleared successfully.\n"));
   
#ifndef DEV_MODE
    std::vector<uint8_t> decrypted_driver = encrypt_executable_file(encrypted_image, encrypted_image.size());
    if (!this->map_bytes(decrypted_driver.data(), arg1, arg2, arg3, free))
    {
        decrypted_driver.clear();
        output->error(xor ("[-] failed to map array of bytes.\n"));
...
#endif  
#ifdef DEV_MODE
    if (!this->map_bytes(encrypted_image.data(), arg1, destroy_header, free))
    {
